...
import { Prisma, Event, Customer, Billing } from "@prisma/client";
...
export class EventServiceBase {
  ...
  async getAddress(parentId: string): Promise<Customer | null> {
  ...
  async getBillingId(parentId: string): Promise<Billing | null> {
    return this.prisma.event
      .findUnique({
        where: { id: parentId },
      })
